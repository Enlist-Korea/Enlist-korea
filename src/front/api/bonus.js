...
function mockCalcScores(payload) {
  ...
  const qualScore =
    payload.qualificationCategory === 'drive'
      ? 45
      : payload.qualificationLabel?.includes('기사')
        ? 50
        : payload.qualificationLabel?.includes('산업기사')
          ? 45
          : payload.qualificationLabel?.includes('기능사')
            ? 40
            : 20;

  const majorScore =
    payload.majorTrack === '비전공'
      ? 26
      : payload.majorLevel?.includes('4학년')
        ? 40
        : payload.majorLevel?.includes('3학년')
          ? 36
          : payload.majorLevel?.includes('2학년')
            ? 32
            : 28;
  ...
  const rawBonus = (payload.bonusSelected || []).reduce(
    (sum, it) => sum + pointOf(it.label),
    0,
  );
