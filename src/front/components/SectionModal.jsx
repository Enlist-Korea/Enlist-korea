...
export default function SectionModal({
  ...
  const allowedCats = qualRule.allowCategories || [
    'nat',
    'ws',
    'gen',
    'drive',
      ...
      <section className="subcard">
        ...
        <div className="form">
          <label className="field">
            ...
            <select
              className="select"
              value={form.qCategory}
              onChange={setVal('qCategory')}
            >
              {allowedCats.includes('nat') && (
                <option value="nat">국가기술자격증</option>
              )}
              {allowedCats.includes('ws') && (
                <option value="ws">일학습병행자격증</option>
              )}
              {allowedCats.includes('gen') && (
                <option value="gen">일반자격증</option>
              )}
              {allowedCats.includes('drive') && (
                <option value="drive">운전면허(차량운전분야 특기 한함)</option>
              )}
              {allowedCats.includes('none') && (
                <option value="none">자격증 미소지</option>
              )}
            </select>
          </label>
  ...
  const renderBonus = () => {
    ...
    const fields = [
      ...
      [
        'bVol',
        '봉사(최근 3년)',
        ['', '8~15시간 (1점)', '16~23시간 (2점)', '24시간 이상 (3점)'],
      ],
